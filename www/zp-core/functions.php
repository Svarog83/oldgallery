...
function getImageMetadata($imageName) {
	...
	if (is_array($imageInfo)) {
		...
		if ($iptc) {
			...
			if (!empty($title)) {
				$result['title'] = iconv( "Windows-1251", "UTF-8", $title );
			}

			/* iptc description */
			$caption= getIPTCTag('2#120');
			if (!empty($caption)) {
				$result['desc'] = iconv( "Windows-1251", "UTF-8", $caption );
			}
			...
			if (!empty($location)) {
				$result['location'] = iconv( "Windows-1251", "UTF-8", $location );
			}
			$city = getIPTCTag('2#090');
			if (!empty($city)) {
				$result['city'] = iconv( "Windows-1251", "UTF-8", $city );
			}
			$state = getIPTCTag('2#095');
			if (!empty($state)) {
				$result['state'] = iconv( "Windows-1251", "UTF-8", $state );
			}
			$country = getIPTCTag('2#101');
			if (!empty($country)) {
				$result['country'] = iconv( "Windows-1251", "UTF-8", $country );
			}
			/* iptc credit */
			...
			if (!empty($keywords)) {
				$result['tags'] = iconv( "Windows-1251", "UTF-8", $keywords );
			}
		}
	}
