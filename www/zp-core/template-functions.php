...
/**
 * Returns the IPTC infromation from the current image
 *
 * @return array
 */
function getImageIPTCData( )  { 
	global $_zp_current_image;
	return $_zp_current_image->getIPTCData();
} 
 ...
 * @param bool $opened if we should show block be default
 */
function printImageMetadata($title='Image Info', $toggle=true, $id='imagemetadata', $class=null, $opened = false) {
	global $_zp_exifvars;
	if (false === ($exif = getImageEXIFData())) { return; }
	$dataid = $id . '_data';
	echo "<div" . (($class) ? " class=\"$class\"" : "") . (($id) ? " id=\"$id\"" : "") . ">\n";
	if ($toggle) echo "<span style=\"cursor:pointer; text-decoration: underline; color: blue;\" title=\"Посмотреть инфо\" onclick=\"toggle('$dataid');\">";
	echo "$title";
	if ($toggle) echo "</span>\n";
	echo "  <table id=\"$dataid\"" . ( $toggle ? " style=\"display: " . ( $opened ? 'block' : 'none' ) . ";\"" : '') . ">\n";
	...
	/*$arr = getImageIPTCData();
	if ( isset (  $arr["2#120"][0] ) )
	{
		$caption = $arr["2#120"][0];
		$caption = iconv( "Windows-1251", "UTF-8", $caption );
		echo "    <tr><td align=\"right\">Описание: </td> <td><strong>&nbsp;&nbsp;$caption</strong></td></tr>\n";
	}*/
